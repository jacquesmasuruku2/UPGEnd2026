...
export default function Grades() {
  ...
  const refreshCourseData = async (courseId: string, allCourses: Course[]) => {
    if (!courseId) return;
    ...
    const { data: historyData } = await supabase.from('grade_history' as any).select('*').eq('course_id', courseId);
  ...
  useEffect(() => {
    ...
    refreshCourseData(selectedCourse, courses);
  ...
  const saveGrades = async () => {
    ...
    for (const [studentId, g] of Object.entries(grades)) {
      ...
      const { error } = await supabase.from('grades').upsert(payload as any, { onConflict: 'student_id,course_id' });
      if (error) failures.push(error.message);
    }
    if (failures.length > 0) {
      toast.error(`Erreur de sauvegarde des cotes (${failures.length}). Vérifiez les données.`);
      return;
    }
    await refreshCourseData(selectedCourse, courses);
    if (transcriptStudent) await openTranscript(transcriptStudent);
  ...
  const saveModifiedGrade = async (studentId: string) => {
    ...
    // Update grade
    const { error: updateError } = await supabase.from('grades').update({ note: newNote } as any).eq('id', g.gradeId);
    if (updateError) {
